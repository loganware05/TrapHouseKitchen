# 🔐 Environment Variables Guide - TrapHouse Kitchen
...
Use `sync: false` in `render.yaml` for variables you'll set manually:
```yaml
- key: STRIPE_SECRET_KEY
  sync: false  # Won't overwrite manual value
```
...
- [ ] Created `.env.example` files (without secrets)
- [ ] Obtained Stripe test keys
- [ ] Obtained Resend API key (optional)
- [ ] Generated strong `JWT_SECRET`
- [ ] Verified local development works
- [ ] Prepared production Stripe keys (when ready)
...
5. Check this guide for required vs optional variables
